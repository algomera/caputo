<div class="p-12 flex flex-col gap-5 bg-color-f7f7f7">
    ...
    @if ($verifiedFiscalCode)
        <div class="flex items-center gap-1">
            <x-icons name="check" @class(['text-color-'.get_color(session('serviceName'))]) />
            <p class="text-color-01a53a text-sm font-medium">{{$verifiedFiscalCode}}</p>
        </div>
    @endif
    ...
    @if ($verifiedPatent)
        <div class="flex items-center gap-1">
            <x-icons name="check" @class(['text-color-'.get_color(session('serviceName'))]) />
            <p class="text-color-01a53a text-sm font-medium">{{$verifiedPatent}}</p>
        </div>
    @endif

    <div class="ml-auto mt-5">
        <x-submit-button wire:click="verifyData" @class(['bg-color-'.get_color(session('serviceName'))])>
            @if ($next) Continua @else Verifica @endif
        </x-submit-button>
    </div>
</div>
