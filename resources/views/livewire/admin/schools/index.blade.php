<div class="h-full bg-white shadow-md rounded-sm p-5">
    <div class="flex flex-col gap-4 border-b pb-5">
        ...
        <p class="text-xl text-color-2c2c2c ">Inserire i dati per la creazione delle filiali dell’autoscuola, sarà possibile aggiungere un’altra filiale, o modificare quelle esistenti dal profilo del Super Admin</p>
    </div>
    ...
    <div class="flex flex-col gap-5 items-start pt-5 h-[calc(100vh-313px)] overflow-y-auto no-scrollbar">
        ...
        <div class="w-full p-5 border shadow">
            <table x-data="{open: null}" class="min-w-full divide-y divide-gray-200 border-b">
                <thead>
                    <tr class="text-center">
                        ...
                        <th scope="col" class="px-3 py-3.5 text-left font-light text-color-545454">&nbsp;
                        </th>
                    </tr>
                </thead>
                ...
                @foreach($schools as $school)
                    <tr class="text-center">
                        <td class="border-r whitespace-nowrap py-4 pl-4 pr-3 font-bold text-color-347af2 sm:pl-6 capitalize">{{$school->code}}</td>
                    ...
                    <tr x-show="open == {{ $school->id }}" class="bg-gradient-to-b from-white to-gray-50 duration-300 shadow-inner transition-all">
                        <td colspan="6" class="px-8 py-5">
                            <button class="px-4 py-2 bg-color-c9defa rounded-md shadow-md text-lg text-color-2c2c2c">+ Aggiungi utente</button>
                            <div class="flex flex-wrap gap-x-8 gap-y-4 mt-5">
                                @foreach ($school->users()->get() as $user)
                                    <div class="relative border p-3 shadow bg-white">
                                        <x-icons name="delete" class="absolute top-3 right-3" />
                                        <p class="capitalize font-medium text-color-545454 mb-2">{{$user->name}}</p>
