<div class="px-14 py-8 flex flex-col gap-4">
    ...
    @if ($stepSkipped && !strpos($stepSkipped, 'accompagnatori'))
        <h1 class="text-3xl font-bold text-color-17489f">{{$stepSkipped}}
            <span class="text-lg">({{$registration->course->name}})</span>
        </h1>

        <div class="flex items-center gap-3">
            <x-container-step>
                @if (strpos($stepSkipped, 'cliente'))
                ...
                @elseif (strpos($stepSkipped, 'Genitore'))
                    <p class="text-xl font-light text-color-2c2c2c">
                ...
                @if (strpos($stepSkipped, 'Genitore'))
    ...
    @elseif ($stepSkipped && strpos($stepSkipped, 'accompagnatori'))
        <h1 class="text-3xl font-bold text-color-17489f">{{$stepSkipped}}
            <span class="text-lg">({{$registration->course->name}})</span>
        </h1>
                    ...
                    @if (count(json_decode($registration->step_skipped)) > 0)
                        @foreach(json_decode($registration->step_skipped) as $step)
                            <tr class="text-center even:bg-color-f7f7f7">
                                <td colspan="3" class="border-r-2 border-color-efefef text-left font-medium px-3 py-4 text-color-2c2c2c">{{get_step($step)}}</td>
                                <td class="border-r-2 border-color-efefef text-left font-medium px-3 py-4 text-color-2c2c2c capitalize">
                                    @if ($step != 'documenti' && $step != 'fototessera')
                                        <div class="w-full flex items-center justify-center">
                                            <button wire:click="$set('stepSkipped', '{{get_step($step)}}')" class="px-4 py-1 text-color-2c2c2c font-medium capitalize rounded-full bg-color-01a53a/30 hover:scale-105 transition-all duration-300">inserisci</button>
                                        </div>
                                    @endif
                                </td>
                            </tr>
